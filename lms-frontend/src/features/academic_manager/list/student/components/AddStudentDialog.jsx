...
const ValidationSchema = Yup.object({
    ...
    name: Yup.string()
        ...
        .min(2, 'Tên quá ngắn')
        .max(80, 'Tên quá dài')
        .matches(/^[\p{L} .'-]+$/u, 'Tên không hợp lệ')
        .required('Bắt buộc'),
    email: Yup.string().email('Email không hợp lệ').required('Bắt buộc'),
    phone: Yup.string()
        .matches(/^(\+?84|0)(3|5|7|8|9)\d{8}$/, 'Số điện thoại VN không hợp lệ')
        .required('Bắt buộc'),
    class: Yup.string().required('Bắt buộc'),
    status: Yup.string().required('Bắt buộc'),
    gpa: Yup.number().min(0).max(4).required('Bắt buộc'),
...
export default function AddStudentDialog({ visible, onHide, defaultValues, onSave }) {
    ...
    return (
        <>
            <Dialog
                header={defaultValues ? 'Edit Student' : 'Add Student'}
                visible={visible}
                style={{ width: '40rem' }}
                onHide={onHide}
                draggable={false}
                    ...
                    {({
                        ...
                        return (
                            <form onSubmit={handleSubmit} className="p-fluid">
                                    ...
                                    <div className="field col-12">
                                        <FloatLabel>
                                            <InputText
                                                ...
                                                className={invalid('name') ? 'p-invalid' : ''}
                                                autoFocus
                                            />
                                    ...
                                    <div className="field col-12 md:col-6">
                                        <FloatLabel>
                                            <InputText
                                                ...
                                                value={values.email}
                                                onChange={handleChange}
                                                className={invalid('email') ? 'p-invalid' : ''}
                                            />
                                    ...
                                    <div className="field col-12 md:col-6">
                                        <FloatLabel>
                                            <InputText
                                                ...
                                                value={values.phone}
                                                onChange={handleChange}
                                                className={invalid('phone') ? 'p-invalid' : ''}
                                            />
                                ...
                                <div className="mt-3">
                                    {footer(isSubmitting, isValid, dirty, handleSubmit, handleCancel)}
                                </div>
                                <ConfirmDialog />
                            </form>
                        );
