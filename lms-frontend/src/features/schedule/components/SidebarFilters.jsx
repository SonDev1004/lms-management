            ...
            <div className="p-d-flex p-flex-column">
                ...
                <div className="p-grid p-mb-2 p-mt-2">
                    <div className="p-col-12">
                        <label className="p-mb-1">Class Type</label>
                        <MultiSelect value={filters.types} options={typeOptions}
                            onChange={(e) => setFilters((s) => ({ ...s, types: e.value }))}
                            placeholder="Select type(s)" display="chip" showClear />
                    </div>
                    <div className="p-col-12">
                        <label className="p-mb-1">Teacher</label>
                        <MultiSelect value={filters.teachers} options={teacherOptions}
                            onChange={(e) => setFilters((s) => ({ ...s, teachers: e.value }))}
                            placeholder="Select teacher(s)" display="chip" showClear />
                    </div>
                </div>

                <div className="p-d-flex p-jc-between p-ai-center p-pt-2">
                    <div className="p-d-flex p-gap-2">
                        <Button icon="pi pi-check" label="Apply" onClick={onApply} className="p-button-apply" />
                        <Button icon="pi pi-times" label="Clear" onClick={onClear} className="p-button-clear" />
                    </div>
                </div>
            </div>
        </Card>
