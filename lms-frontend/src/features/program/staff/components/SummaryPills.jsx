...
export default function SummaryPills({ stats = {}, entity }) {
    const detected =
        entity ||
        (('avgCourses' in stats) || ('avgModules' in stats) || ('totalPrograms' in stats)
            ? 'programs'
    ...
    const metricValue =
        detected === 'programs'
            ? (stats.avgCourses ?? stats.avgModules ?? stats.avgEnrollment ?? 0)
            : detected === 'courses'
    ...
    const labels =
        detected === 'programs'
            ? {
                ...
                inactive: 'Inactive Programs',
                metric:
                    ('avgModules' in stats && stats.avgModules != null)
                        ? 'Avg Modules / Program'
                        : 'Avg Courses / Program',
            }
            : detected === 'courses'
    ...
    const isPercent =
        detected === 'students'
            ? true
            : (detected === 'courses' && (metricValue <= 1 ? true : metricValue > 1));

    const displayMetric = metricValue <= 1 ? (metricValue * 100) : metricValue;
...
SummaryPills.propTypes = {
    ...
    entity: PropTypes.oneOf(['students', 'courses', 'programs']),
};
