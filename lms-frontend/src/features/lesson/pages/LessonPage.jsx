...
export default function LessonPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setLoading(true);
        const items = await lessonService.getBySubject(Number(subjectId), {
          page,
          size,
        });
        if (!ignore) {
  ...
  const openDoc = (item, doc) => {
    setSelected({ ...item, activeDoc: doc });
    setVisible(true);
  };
  const onHide = () => {
    setVisible(false);
  ...
  if (error)
    return <div>Lỗi tải bài học: {String(error?.message || error)}</div>;
  ...
  return (
    <>
      ...
      {!loading && syllabusData.length === 0 && (
        <div className="text-center text-muted my-3">Chưa có bài học nào</div>
      )}
      <Paginator
        first={first}
        rows={size}
